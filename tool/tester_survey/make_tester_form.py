...
A(" *   6. ⚠️ 보내기 전에 **시크릿 창(로그아웃 상태)**으로 테스터용 주소를 한 번 연다.")
A(" *      만든 사람 화면에서는 늘 멀쩡해 보인다 — 로그인 화면이 뜨면 아래 설정을")
A(" *      확인한다: 설정 → 응답 → \"로그인 필요\"/\"조직 내 제한\"/\"이메일 수집\"")
A(" *")
...
A("")
A("  // ⚠️ 로그인 없이 열리게 한다 — 지정하지 않으면 구글 기본값을 따르는데,")
A("  //    워크스페이스 계정에서 만들면 기본이 \"조직 내 사용자로 제한\"이라")
A("  //    만든 사람 화면에서는 멀쩡하고 테스터에게만 로그인 화면이 뜬다.")
A("  //    2026-08-18에 실제로 겪었다 — 배포하려던 주소가 HTTP 401이었다.")
A("  try {")
A("    form.setRequireLogin(false);          // 워크스페이스 계정에서만 동작한다")
A("  } catch (e) {")
A("    Logger.log('setRequireLogin 실패(개인 계정이면 정상): ' + e);")
A("  }")
A("  form.setCollectEmail(false);            // 이메일 수집은 로그인을 강제한다")
A("  form.setLimitOneResponsePerUser(false); // 1회 제한도 로그인을 강제한다")
A("")
...
A("  Logger.log('');")
A("  Logger.log('⚠️ 보내기 전에 시크릿 창으로 테스터용 주소를 한 번 열어 보세요.');")
A("  Logger.log('   로그인 화면이 뜨면 설정 → 응답에서 로그인 요구를 꺼야 합니다.');")
A("}")
