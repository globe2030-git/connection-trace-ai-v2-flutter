...
if [ "$TARGET" = "apk" ] || [ "$TARGET" = "appbundle" ]; then
  # 2026-08-26 사용자 결정: **32비트(android-arm)도 뺀다.**
  #
  # 되돌리는 비용이 한쪽으로 기울어 있다 — 빼고 시작했다가 나중에 넣는 것은
  # 무해하지만(그때부터 설치 가능해질 뿐), 넣고 시작했다가 빼면 **이미 깔린
  # 32비트 이용자의 업데이트가 끊긴다.**
  #
  # 그리고 지금 32비트를 **아무도 검증하지 않는다** — 테스트 기기가 폴드
  # (64비트)뿐이라, 명함 테두리 검출(OpenCV)·OCR(ML Kit)·카메라 세 네이티브가
  # 32비트에서 어떻게 도는지 볼 사람이 없다. 검증할 수 없는 대상에게 파는
  # 것보다 안 파는 쪽을 택했다.
  #
  # ⚠️ 이용자가 받는 양은 이것으로 줄지 않는다. AAB는 기기마다 자기 ABI만
  # 보내므로 arm64 이용자는 전부터 31.7MiB였다(추가 483 실측). 줄어드는 것은
  # **AAB 파일 크기와 테스터 APK**다.
  ABI_FLAG="--target-platform=android-arm64"
fi
