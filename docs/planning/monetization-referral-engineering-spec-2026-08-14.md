...
| AC-2 | 충전 티어 4개, 회당 단가 단조감소 | ⚠️ **2026-08-18: `{1000,20}`으로 확정되면서 단조감소가 깨졌다**(회당 ₩50→90.9→83.3→76.9). 상위 3티어 재산정 전까지 이 인수 기준은 **충족 불가**다 — 권고는 전체 2배(`{1000,20}/{3000,66}/{5000,120}/{10000,260}`)이고 사용자 확인 대기. 아래는 개정 전 기준이다: `config/billing.tiers`에 정확히 4행: `{1000,10}/{3000,33}/{5000,60}/{10000,130}`, 나머지 기존 3티어(30000/50000/100000, 2026-08-11 결정분)는 `active:false`로 비활성(데이터 변경, 관리자 콘솔에서 즉시 가능 — **코드 배포 불필요**). `BillingConfigRepository.fetchConfig()`가 `active`만 필터하므로 앱은 자동으로 4개만 그린다(코드 변경 0줄). |
