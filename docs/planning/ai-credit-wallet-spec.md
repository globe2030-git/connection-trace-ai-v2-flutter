...
> (저장소 밖 — `/Volumes/Work/Claude/` 아래, 이하 "인계 스펙")가 이 문서의
...
브리핑을 처음 누르는 순간까지 잔액이 0으로 보이면 "무료 20회" 안내가
거짓말이 된다. `AiUsageService.fetch()`는 화면 진입 시 바로
`users/{uid}.aiUsage`를 읽으므로, 로그인 직후 지급을 끝내 둬야 사용자가
AI 화면에 처음 들어갔을 때부터 "잔여 20회"가 보인다.

⚠️ **회차는 2026-08-18에 10 → 20으로 바뀌었다**(사용자 확정, 하루 한도 폐지와
한 쌍 — `monetization-referral-implementation-spec-2026-08-14.md` §1 개정 블록).
실제 값은 `config/billing.freeCredits`이고, 서버가 못 읽었을 때의 폴백이
`freeGrant.ts`의 `DEFAULT_FREE_CREDITS`다.
...
**P0-11과의 관계 — 2026-08-18 갱신**: 사용자가 **일·월 회차 한도 제도
자체를 폐지**하기로 확정했다(충전형 확정의 귀결. 잔액이 상한이므로 회차
제한이 이중으로 걸릴 이유가 없다). 따라서 P0-11은 *"20을 10으로 되돌리는
일"*이 아니라 **P1-5에서 `DAILY_LIMIT`/`MONTHLY_LIMIT`을 함께 제거하는
일**로 흡수됐다. **"폐기"로 읽지 말 것** — 상수가 코드에 남아 있는 동안은
여전히 동작한다.

⚠️ **제거는 지갑 배포와 같은 시점이어야 한다.** 먼저 지우면 그사이 **아무
상한도 남지 않는다**(판정이 `dailyCount >= DAILY_LIMIT || monthlyCount >=
MONTHLY_LIMIT`이라 하루 한도만 지우면 월 100회가 남지만, 월 한도까지
없애면 남는 것이 없다). 함께 볼 자리: `index.ts:404` 이하의 일/월 각각
다른 에러 메시지 분기(잔액 부족 메시지로 대체돼야 한다), `index.ts:974·976`이
클라이언트로 내려보내는 `dailyLimit`/`monthlyLimit`.

반대로 wallet 전환이 늦어져 **reset 모드로 먼저 출시**하게 되면, 그때는
이 한도가 유일한 상한이므로 값을 그대로 두거나 다시 검토해야 한다. ⚠️ **`chore/p0-11-
