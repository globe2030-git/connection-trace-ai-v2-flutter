...
> (추가 104·108·109·110)와 [`sessions/2026-08.md`](./sessions/2026-08.md) "0-1 ~ 0-8" 참고.
...
| 9 | Android 서명 키 바꾸니 Google 로그인 실패 | "로그인에 실패했습니다"(Android만, iOS는 정상) | 새 키의 **SHA-1을 Firebase에 등록**(디지털 지문 추가). Play 배포 시 **Play 앱 서명 키 SHA-1**도 추가 |

> **Android Google 로그인 & SHA-1**: Google Sign-In은 (패키지명 + 서명 키 SHA-1)을
> 서버에서 검증한다. 그래서 **서명 키가 바뀔 때마다**(debug → 업로드 키 → Play
> 앱 서명 키) 해당 SHA-1을 Firebase Console(프로젝트 설정 → Android 앱 → 디지털
> 지문 추가)에 등록해야 그 빌드에서 로그인이 된다. 서버 측 검증이라 등록 후
> 재빌드는 불필요. keytool 지문 추출 시 한글 로케일 버그가 나면
> `keytool -J-Duser.language=en -J-Duser.country=US ...`로 우회. iOS는 SHA-1이
> 아니라 번들 ID 기반이라 무관.
