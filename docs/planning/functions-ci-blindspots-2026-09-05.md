...
## 4. 🚨 `node` 가 없었다 — 그리고 **원래 없던 것이 아니었다**

> ✅ **복구됐다 (2026-09-05).** globe2030님이 `brew install node` 로 되살리셨다.
> `node v26.8.1` · `npm 11.19.0`.
>
> 🚨 **원인은 「새 노트북에 원래 없던 것」이 아니다** — globe2030님이
> `brew uninstall firebase-cli` 를 실행하셨는데 **그것이 node 를 함께 지웠다**
> (`/opt/homebrew/bin/firebase` 의 shebang 이 `#!/opt/homebrew/opt/node/bin/node`
> 였고, formula 가 node 를 의존성으로 물고 있었다 — PM 실측).
>
> ⭐ **그러니 아래 관측은 「그 순간에는 맞았다」이고, 원인 추정이 틀렸다.**
> 「없다」는 맞았고 **「원래 없다」가 아니라 「몇 시간 전에 사라졌다」**였다.
> 📌 오늘 여러 번 나온 *"조회한 값은 조회한 순간의 것"* 이 **환경에도 성립한다.**
>
> ⚠️ **그래서 런북 「1-1 새 노트북에서 걸린 것」에는 안 넣는다** — 성격이 다르다.
> 대신 *"`brew uninstall firebase-cli` 가 node 를 함께 지운다"* 를 적을 자리가 맞다.
...
```
✅ 테스트가 몇 건 도는가        **260건** (node v26.8.1 실측 · pass 260 fail 0 · 87ms)
                              [추가 679] 의 251건은 그 시점 값이고 #830 에서 9건이 늘었다
✅ 덮인 18개가 **무엇을 재는가**  변조를 넣어 봤다 — 아래 5번
...
## 5. ✅ 깨뜨려 봤다 — 덮인 것이 실제로 지키는가 (2026-09-05)

### 🚨 먼저 — **첫 시험은 통째로 무효였다**

```
1차   ct-doc-fix 워크트리에 node_modules 가 없었다
      → tsc 가 없어 npm test 가 **변조와 무관하게 항상 실패**
      → 내 스크립트는 그것을 전부 「✅ 잡았다」로 셌다
      → **19/19 전부 잡힘** 이라는 결과가 나왔고 **전부 무효**였다
```

⚠️ **같은 날 `verify_rules.py` 에 *"대조군이 없으면 거저 통과한다"* 고 적어 놓고,
내 시험에는 대조군을 안 뒀다.** 🚨 **규약을 쓴 사람이 그 자리를 밟는 것**이 오늘만
저장소에서 세 번째다.

⭐ **그래서 2차부터는 「변조 없이 돌려 종료 0 을 확인」하는 것을 첫 줄에 박았다.**

### 방법

```
대조군      변조 없이 npm test → 종료 0 확인 (261건 통과)  ← 통과해야 시험을 시작한다
변조 순서   ① 첫 return false; → return true;
            ② 첫 비교 연산자 뒤집기 (< ↔ <= · > ↔ >=)
            ③ 첫 숫자 상수 +1  또는  첫 문자열 상수 뒤에 X
되돌리기    매번 원본으로 복원하고 git status 로 확인
```

📌 **모듈당 하나씩만 넣었다** — 전수가 아니다. 아래 ⬜ 참고.

### 결과 — 19개 중 **판정에 해당하는 것은 전부 잡혔다**

| | 모듈 | 판정 |
|---|---|---|
| ✅ 15 | adminAuth · briefingPrompt · chunk · creditGrant · deviceLedger · freeGrant · kakaoUnlink · phoneOtp · phoneOtpSender · pilotEvents · purchases · referralCode · socialAuth · usageReset · walletCredits | 바로 빨개짐 |
| ✅ 1 | `adminEmails` | `npm test` 는 안 잡지만 **`tool/check_admin_sync.py` 가 잡는다** — 변조 시 **종료 1**(`ci.yml:127` 에서 돈다) |
| ✅ 1 | `tombstoneCleanup` | `400→401` 이 **무의미한 변조**였다(Firestore 상한 500 아래라 여전히 안전). **`400→600` 으로 하니 잡는다** |
| 🟡 2 | `cardPhotoCleanup` · `phoneRecordCleanup` | `errorType: (e as Error)?.name ?? "Unknown"` 꼴 — **판정이 아니라 보고용 기본값**이다. 안 잡는 것이 자연스럽다 |

⭐ **`verify_rules.py` 때와 정반대 결과다.** 거기서는 *"일반 로그인 사용자"* 케이스가
**아무것도 증명하지 않는다**는 것이 드러났는데, 여기서는 **약한 변조가 드러났다.**

📌 **그 구분이 중요하다.** 「안 잡혔다」에는 두 가지가 있다.
...
🚨 **둘을 안 가르면 멀쩡한 테스트를 「구멍」으로 보고한다.** 이번에 `tombstoneCleanup`
이 그 자리였다 — 한 번 더 재지 않았으면 **없는 구멍을 적을 뻔했다.**

### ⚠️ 그리고 파이프가 종료 코드를 삼켰다

`check_admin_sync.py` 를 처음 잴 때 `python3 … | tail -3` 로 돌려 **종료 0** 이
나왔고, *"검출은 하는데 CI 는 통과한다"* 는 **가짜 결함**을 잡을 뻔했다.

```
❌ python3 tool/check_admin_sync.py | tail -3 ; echo $?   → tail 의 코드라 항상 0
✅ python3 tool/check_admin_sync.py > 로그 2>&1 ; echo $?  → 깨끗할 때 0 · 변조 시 1
```

📌 **`CLAUDE.md` 4-2 절이 `git checkout` 에 대해 적어 둔 그 함정이 여기서도 났다.**
그 절은 브랜치 이야기로 적혀 있어 **「파이프 일반의 문제」로 안 읽힌다.**

### ⬜ 이 시험이 여전히 못 본 것

```
⬜ 모듈당 **하나씩**만 변조했다 — 줄 단위 전수가 아니다
⬜ 어느 자리를 변조할지는 **자동으로 고른 것**(첫 return false · 첫 비교 · 첫 상수)이라
   **가장 중요한 판정을 골랐다는 보장이 없다**
⬜ index.ts 는 **아예 제외했다** — 테스트가 없어 변조해도 잴 것이 없다
```
