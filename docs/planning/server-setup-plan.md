...
**호출 1회당 비용** ≈ (1,200 × $1.50 + 400 × $7.50) / 1,000,000
= ($1,800 + $3,000) / 1,000,000 ≈ **$0.0048** → 여유를 두어 **약 $0.005/회**로
반올림.

**월 비용 = 사용자 수 × 사용자당 월평균 호출 수 × $0.005** — 상세
표는 11번 섹션 "AI 프록시 비용 추정"에 있다. 요약하면: 사용자가
평균적으로 쓸 것으로 예상되는 수준(월 15회)에서는 1만 명 규모에서도
월 $750 수준으로, Firestore/Auth 인프라 비용보다 AI 비용이 더 커질
수 있다는 점을 미리 인지해 둘 것 — 상한(14.4절)과 예산 알림(11번
섹션)을 반드시 함께 배포해야 하는 이유다.

**Cloud Functions 자체의 실행 비용**(AI 프록시 함수 호출 자체, AI
API 요금과는 별개)은 무료 한도(월 200만 회)를 이 앱 규모에서 초과할
가능성이 거의 없어 사실상 $0으로 봐도 무방하다(11번 섹션 표 참고).

**여러 제공사를 동시에 서버가 부담하면**: Claude/OpenAI의 상대적으로
비싼 모델(예: Claude Sonnet 계열, GPT-4o 계열)을 함께 제공하면 위
추정치의 5~10배까지 비용이 뛸 수 있다(제공사·모델별 단가 차이가 큼).
**⚠️ 사용자 확인 필요**: 서버가 실제로 몇 개 제공사를 부담할지(14.6절
권장안은 Gemini 단독).
