...
- **구체적인 상한값은 실제 사용 데이터가 쌓이기 전까지는 추정치다** —
  ⚠️ **2026-08-18 갱신: 이 절의 일/월 한도 설계는 폐기됐다**(추가 303).
  충전형이 확정되면서 회차 한도 대신 **충전 잔액이 상한**이 된다. 아래
  서술은 지갑 전환(P1-5) 이전 구조의 기록으로 읽을 것.
...
### 15.8 기존 사용자 사진 소급 업로드 여부 — 🟡 상위 결정은 났고, 소급 방식만 남음

> **2026-08-15 사용자 확정(추가 218)**: **명함 이미지를 서버에 저장한다.**
> 단 **수집 목적은 "복원·서비스 제공"(계약 이행)으로만 고지**하고, **명함
> 인식 품질 개선에 쓰는 것은 설정에서 켠 사용자 것만**으로 분리한다.
>
> 근거는 리멤버 개인정보처리방침 원문 확인 — 리멤버도 "회원이 등록한 **인맥의**
> 명함 이미지"를 수집 항목에 명시하고 제3자 정보 처리 근거를 **개인정보 보호법
> §15①4호(계약의 이행)**로 잡는다. 다만 리멤버 방침에도 "인식 알고리즘 개선"
> 목적은 없어서, 그 부분만 별도 동의로 뗐다.
>
> **선행 조건(미착수)**: 개인정보처리방침 개정(수집 항목에 명함 이미지 추가,
> 보유기간, Cloud Storage 위탁) + **7일 고지**. 방침이 게시되기 전에 이미지를
> 서버로 올리는 코드를 켜면 안 된다.
>
> **아래 "소급 업로드" 방식(자동 vs 옵트인)은 2단계 착수 시 이 결정 위에서
> 다시 정한다** — 신규 저장과 기존 사진 소급은 별개 판단이다.
