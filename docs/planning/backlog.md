...
- **(2026-08-02 결정, 사용자 결정 필요)** 주소 입력을 자유 텍스트 + 저장
  시점 지오코딩 방식에서 **도로명주소 검색/자동완성**(카카오/다음 우편번호
  서비스 같은 패턴)으로 교체 — 검색 결과 목록에서 실제 주소를 선택하는
  구조라 애초에 지오코딩 실패 자체가 거의 안 생김. "위치를 찾을 수 없는
  주소" 안내(`_showUnresolvableAddressDialog`)를 만들 필요가 크게 줄어드는
  더 근본적인 해결책. 진행하려면 **카카오 디벨로퍼스 앱 등록 + API 키
  발급**이 먼저 필요(Gmail OAuth 때와 비슷한 유형이지만 훨씬 간단하고
  무료) — 사용자 계정 설정이라 코드만으로는 진행 불가. 사용자가 "이것도
  해야할 업무로 기록해두고 리마인드해줘"라고 명시적으로 요청.
