...
>
> ```
> ① 시크릿          1단계 — 🚨 PHONE_HASH_SALT 를 포함한다 (아래 참고)
> ② 함수 배포        2단계
...
### 1단계 — 시크릿 🔒 ⚠️ **6개가 아니라 7개다 (2026-09-05)**

> 🚨 **`PHONE_HASH_SALT` 가 이 목록에서 빠져 있었다.** 그리고 **그것이 없으면
> 배포 자체가 안 된다** — PM 이 2026-09-05에 실제로 배포하다 막혔다(**PM 실측**,
> *"소스 로딩 단계에서 걸린다 · 함수를 좁혀도 같다"*).
>
> **코드로 확인한 것**(이 문서를 쓴 세션): `index.ts:183` 이
> `defineSecret("PHONE_HASH_SALT")` 로 모듈 최상위에 선언하고, `:2707` 이
> `secrets:` 에 넣는다. 즉 **번호 확인 함수가 이 시크릿을 요구한다.**
>
