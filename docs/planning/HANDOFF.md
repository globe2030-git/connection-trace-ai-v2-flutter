...
| # | 무엇 | 왜 지금인가 |
|---|---|---|
| ~~㉠~~ | ~~무료 20회가 최저 티어를 삼킨다~~ → ✅ **해소됨(2026-08-18, 추가 303): 충전 사다리 전면 개정.** **7단계 전부**: 30 / 100 / 200 / **450** / 1,500 / 2,500 / 5,000회(₩1천·3천·5천·1만·3만·5만·10만). 회당 ₩33.3→30.0→25.0→**22.2**→20.0→20.0→20.0 — **활성 4단계는 단조감소**다(₩10,000을 400→450으로 올려 ₩5,000 두 번과 같아지던 평평한 구간을 없앴다). ⚠️ **상위 3단계는 아직 `active:false`, 활성화 여부 미결**이고 서로 ₩20.0 동률이라 켤 때 재손질이 필요하다. 무료 20 < 유료 최저 30이라 유료를 살 이유가 분명해졌고, 최저만 올렸을 때 생기던 역전(₩1,000 세 번=90회 vs ₩3,000=33회)도 함께 풀렸다. ⚠️ **남은 것: `config/billing.tiers` 반영**(관리자 콘솔 데이터, 코드 배포 불필요) + **스토어 SKU 등록** | 스토어에 올린 뒤 회수를 바꾸는 건 훨씬 번거롭다. **아직 등록 전** |
