...
1. **v1 출시 스코프 — 무료 단독 출시 vs 구독(유료) 포함 출시.** 손익분석
   ...
   **⚠️ 2026-08-09 갱신(추가 126)**: 사용자 제안으로 **회차를 파는 두 안**을
   같은 문서 **13·14번 섹션**에 추가했다 — E안(무료 5회 + 충전형), F안(회차
   구독, ₩1,000·10회부터 단계마다 +10%). 세 변형 모두 평균·최악 양쪽에서
   흑자이며, 권고는 **v1 무료 출시 → v1.1 충전형 → 실측 후 회차 구독**이다.
   충전형은 소비형 인앱결제라 위 5건 중 가장 무거운 둘(갱신 상태 동기화,
   결제 30일 전 사전 고지)을 비껴간다. **결정 시 함께 정할 것 3가지**:
   ① 회차 사다리 10/33/60/130 확정, ② 무료 5회를 매월 줄지(매월이면 순이익
   1/3.2), ③ 주 단위 한도 도입 여부(문서는 **도입하지 않을 것**을 권고 —
   13-5). 티어 회차 확정 전에 `aiAuditLogs` 실측을 먼저 볼 것.
