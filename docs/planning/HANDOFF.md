...
| P1-39 | **같은 계정 여러 기기 간 동기화** → 🟡 **추가형 병합 완료(2026-08-09, 추가 117)** | 로그인 시 `mergeFromServer`가 **서버에만 있는 명함(id 기준)을 로컬에 더한다**(로컬은 절대 안 지움 → 데이터 손실 0). A기기 새 명함이 B기기에 안 뜨던 핵심 불만 해소. `auth_gate` 정상 로그인 경로에만 연결(계정 전환 "유지" 경로엔 미연결 — 데이터 혼입 방지). 순수 로직 `computeServerAdditions` 분리+테스트 3건. **⚠️ 남은 것**: 다른 기기에서 **삭제**된 명함은 아직 로컬에서 안 사라짐(삭제 전파=tombstone 별도 작업), 크로스기기 전화번호 중복(P1-40)도 여기선 안 잡음. **실기기 2대 검증 권장** | 잔여: 삭제 동기화 | 개발 |
