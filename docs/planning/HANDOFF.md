...
1. ~~**v1 출시 스코프 — 무료 단독 출시 vs 구독(유료) 포함 출시.**~~
   ...
   > 무료분 원가 $0.023 → **$0.046**. **손익분기 전환율(0.14%) 재계산 필요** —
   > `business/pnl-analysis-freemium.html`은 아직 10회 기준이다.
...
| AI 하루 한도 | `functions/src/index.ts:71` **`DAILY_LIMIT = 20`** — **P0-11은 2026-08-18에 폐기**(하루 한도 제도 자체를 없애기로 확정). ⚠️ 값은 **지갑 배포 전까지 그대로 둔다** — 먼저 지우면 상한이 사라진다 |
...
> 폐기됐다** — 사용자가 하루 한도 제도 자체를 없애기로 확정했다(무료 회차
> 10→20과 한 쌍). 코드의 `DAILY_LIMIT = 20`은 지갑 배포 전까지 그대로 둔다.
>
...
| ~~**P0-11**~~ | ~~**AI 하루 호출 한도를 20 → 10으로 되돌리기**~~ → ❌ **폐기(2026-08-18 사용자 확정)** — *"지금 모델의 하루 무료 호출 한도는 없는 것으로 한다."* 하루 한도 제도 자체가 없어지므로 되돌릴 값이 없다. ⚠️ **다만 지금 지우면 안 된다** — 지갑 모델 배포 전에 `DAILY_LIMIT`부터 없애면 무료 상한이 사라져 비용이 샌다. **제거는 P1-5(지갑 전환)와 함께.** 아래는 폐기 전 기록이다: **재오픈(2026-08-10, 추가 148)**. 추가 147에서 10으로 되돌렸다가, **직원 테스트 기간 동안 20으로 되돌려 놓았다** — 테스터가 하루에 여러 번 눌러 봐야 하는데 10회에 막히면 "AI가 안 된다"는 제보만 쌓이고 정작 봐야 할 것을 못 본다. **테스트가 끝나면 반드시 10으로.** 안 되돌리면 1인당 비용 상한이 설계의 2배가 된 채로 출시된다. **두 파일을 함께**: `functions/src/index.ts`의 `DAILY_LIMIT`, `lib/core/services/ai_briefing_service.dart`의 `dailyLimit`. 바꾼 뒤 `firebase deploy --only functions` 필수 | **신규(2026-08-08, 추가 100)**. `functions/src/index.ts`의 `DAILY_LIMIT`이 원래 10이었는데, 사고 토큰 단가를 단계별로 재려면 하루에 여러 번 호출해야 해서 사용자 확인 후 **테스트용으로 20으로 올려 뒀다.** App Check 강제 이후라 외부 유출 위험은 없지만, **출시 전에 반드시 10으로 되돌릴 것** — 안 되돌리면 1인당 비용 상한이 설계의 2배가 된 채로 나간다. 코드에도 🚧 주석으로 표시해 뒀다 | 소 | 개발 |
