...
| P0-2 | Apple 로그인 — **Firebase 콘솔 설정만 남음** | **2026-08-06 원인 확정(추가 83)**: 코드는 정상이고 iOS 실기기에서 Apple 버튼이 뜨고 Apple 인증 화면까지 진행되는 것을 확인했다. 실패 원인은 **Firebase에 Apple 제공사가 등록돼 있지 않은 것** — Identity Toolkit 관리 API 조회 결과 등록된 제공사가 `google.com` 하나뿐이다. 남은 작업: ① Apple 개발자 포털에서 App ID에 Sign in with Apple 활성화 + 키(.p8) 발급 ② Firebase 콘솔 Authentication에 Services ID·Team ID·Key ID·키 등록 ③ Apple 버튼 HIG 스타일링 | 소 | **사용자**(콘솔) + 개발(스타일링) |
