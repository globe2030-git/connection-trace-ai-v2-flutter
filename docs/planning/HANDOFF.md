...
| | 실측값 |
|---|---|
| `main` 최신 | `dac6424` (경로 정리, 추가 299) ← `fadbc00` (테스터 빌드 8, 추가 298) |
| 앱 버전 | `pubspec.yaml` **1.0.0+8** |
| 열린 PR | **없음** |
| 미병합 브랜치 | **`feat/ai-credit-wallet`** 하나 — 지갑/과금. **의도적 보류다**(CLAUDE.md 6절: 과금은 스토어 출시 전에 테스트·배포하는 것이 관문이고, main에 얹어 두면 다른 이유의 `firebase deploy`에 딸려 나갈 위험이 있다) |
| 워크트리 | `-docs` · `-feature`(둘 다 detached) · `-wallet`(`feat/ai-credit-wallet`). 전부 미커밋 0건 |
| AI 하루 한도 | `functions/src/index.ts:71` **`DAILY_LIMIT = 20`** — P0-11은 2026-08-18에 **P1-5로 흡수**(하루 한도 제도 자체를 없애기로 확정). ⚠️ 값은 **지갑 배포 전까지 그대로 둔다.** 월 한도 100은 그대로 걸리므로 먼저 지워도 "비용 폭주"는 아니고 **버스트가 열린다** |
