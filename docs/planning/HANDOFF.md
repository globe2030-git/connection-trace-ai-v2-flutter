...
| P1-39 | **같은 계정 여러 기기 간 동기화 없음** | **신규(2026-08-08, 추가 106)**. 복원이 `restoreFromServerIfEmpty`로 "로컬이 비어 있을 때만" 일어나 **백업이지 동기화가 아니다.** 기기 A에서 추가한 명함이 기기 B에 나타나지 않고, 양쪽에서 추가하면 서버에는 합쳐지지만 각 기기는 자기 것만 본다. 사용자는 같은 계정이니 동기화될 것으로 기대한다. 기종이 달라도(아이폰+안드로이드) 동일 | 중~대 | 개발 |
