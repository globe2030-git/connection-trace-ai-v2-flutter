...
| P1-38 | **Apple 토큰 폐기(revoke)** → 🟢 **배포 완료, 실기기 최종검증만 남음(2026-08-09, 추가 114·126, PR #44)** | 로그인 시 authorizationCode를 서버가 refresh_token으로 교환해 `appleAuth/{uid}`(클라이언트 접근 불가)에 보관, 탈퇴 시 `onUserDeletedCleanup` 트리거가 Apple `/auth/revoke` 호출. ES256 JWT는 Node crypto로 직접 서명, client_id=번들ID. **✅ 2026-08-09 배포 완료**: `APPLE_SIGNIN_KEY` 시크릿 설정(`~/keys/AuthKey_UUYAKPD4S7.p8` → Secret Manager v1) + `functions,firestore:rules` 배포. `storeAppleRefreshToken` 신규 생성, `onUserDeletedCleanup` 갱신. 클라이언트 호출부는 이미 설치 빌드(c9cee96)에 포함. **잔여: 실기기 E2E 검증**(아이폰 Apple로그인→계정삭제→Apple ID 설정 "Apple로 로그인" 목록에서 앱 사라짐 확인). ⚠️ 기존 Apple 세션 사용자는 **한 번 다시 Apple 로그인**해야 refresh_token이 `appleAuth/{uid}`에 저장됨 | 잔여: 실기기 검증 | 개발/사용자 |
