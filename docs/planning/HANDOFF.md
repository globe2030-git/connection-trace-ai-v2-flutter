...
| P1-40 | **전화번호 중복 검사 없음** | **신규(2026-08-08, 추가 106)**. 같은 사람 명함을 두 번 스캔하면 두 건으로 들어간다(`grep` 결과 중복 검사 코드 없음). P1-39와 겹치면 더 나빠진다 — 두 기기에서 각각 등록하면 서버에 2건이 쌓인다. **오늘 확인된 구조 문제 셋 중 가장 싸고 효과가 크다**(저장 직전 같은 번호 확인) | 소 | 개발 |
