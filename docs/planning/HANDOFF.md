...
| # | 무엇 | 왜 지금인가 |
|---|---|---|
| ~~㉠~~ | ~~무료 20회가 최저 티어를 삼킨다~~ → ✅ **결정됨(2026-08-18): ₩1,000 티어를 10 → 20회로 상향.** 🚨 **그런데 이게 사다리를 뒤집었다** — 회당 ₩50인데 ₩3,000은 ₩90.9라 **많이 살수록 손해**다. **상위 3티어 재산정이 새 미결**이고, 권고는 전체 2배(`20/66/120/260`, 회당 ₩50→45.5→41.7→38.5)다. 확정 전 SKU 등록 금지 | 스토어에 올린 뒤 회수를 바꾸는 건 훨씬 번거롭다. **아직 등록 전** |
