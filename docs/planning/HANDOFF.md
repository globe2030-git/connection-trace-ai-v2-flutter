...
| **P1-41** | **Gmail 기능 차단 3종(결정-2 이행)** | **신규(2026-08-09, 추가 128)**. ① `communication_source_sheet.dart:61-66`의 "Gmail에서 가져오기"를 "준비 중"으로 잠금(코드·서비스는 **삭제하지 않고 보존**), ② ⚠️ **Google Cloud Console 동의 화면에서 `gmail.readonly` 범위 제거** — 버튼만 숨기고 범위를 남기면 "쓰지도 않는 제한된 범위를 요구한다"는 이유로 검증에서 반려될 수 있다(이 항목이 빠지면 차단의 의미가 없다), ③ `docs/legal/privacy-policy.html`의 Gmail 서술 정리. **베타 배포 전에 끝낼 것** | 소(①③) + 사용자 콘솔 작업(②) | 개발 + 사용자 |
