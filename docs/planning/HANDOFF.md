...
않는다"고 쓰여 있는데 **코드는 아직 저장 중**이다. 코드 수정 전에 게시하면
허위 기재가 된다(각 HTML 상단에 화면 미표시 주석으로 박아 둠). 같은 이유로
...
3. **보호책임자 = 최우진(대표이사) + `privacy@creamhouse.net`**.
...
| P0-5 | ~~개인정보처리방침 담당자·전용 문의메일 정식화~~ → **법적 고지 문서 게시** | **2026-08-05 갱신: 문서 작성은 완료**(0-2 섹션). 보호책임자·문의메일 임시값은 최우진(대표이사) + `privacy@creamhouse.net`으로 정식화했고, 방침 v2.0 전부개정·이용약관·접근권한 안내·계정삭제 안내까지 작성 완료. 남은 것은 ① **`privacy@creamhouse.net` 메일 계정 생성**(사용자) ② C안 코드 구현 후 Firebase Hosting 배포 ③ 스토어 콘솔에 URL 등록 | 소 | 사용자(메일 개설) + 개발(배포) |
...
| P1-14 | **좌표 C안 구현 — 명함 좌표를 서버 백업에서 제외** | **P0-5 게시의 선행조건.** `contact_model.dart:112-113`의 lat/lng가 `toJson()`에 포함돼 Firestore에 저장 중인데, 방침 v2.0은 C안 기준("서버에 보관하지 않음")으로 작성돼 있다 — 코드를 안 고치고 게시하면 허위 기재. 복원 후 주소 재지오코딩 + 기존 Firestore 문서의 좌표 마이그레이션 삭제 필요. ⚠️ 새 기기 복원 직후 거리 계산이 불가해지므로 일괄 재지오코딩 설계 필요(`address_geocoding_service.dart`에 10초 타임아웃·실패 fallback 구조가 이미 있어 확장 가능) | 중 | 개발 → QA |
