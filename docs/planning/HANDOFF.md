# 개발 인수인계 문서 (2026-08-05 기준 — 최신 상황은 "0-4" 섹션, 우선순위는 "3. 해야 할 일" 참고)
...
| P0-8 | **Firestore 규칙 필드 단위 강화** | **신규(2026-08-06, 추가 82). AI 프록시 배포(P1-8) 전 필수.** `firestore.rules`가 `/users/{uid}`에 `allow read, write: if isOwner(uid)`로 **문서 전체** 쓰기를 허용하는데 한도 카운터(`aiUsage`)가 같은 문서에 있다 → 사용자가 SDK로 `dailyCount: 0`을 직접 써넣으면 **일 10회 한도가 무한 초기화**되고 그 비용은 **회사 명의 유료 Gemini 키**로 나간다. `encryptionKeyB64`도 덮어쓸 수 있어 **키가 바뀌면 서버 백업 전체가 영구 복호화 불능**(악의보다 버그로 터질 확률이 높음). `aiUsage`는 클라이언트 쓰기 금지(서버 Admin SDK는 규칙 우회), `encryptionKeyB64`는 최초 1회 생성만 허용 | 중 | 개발 |
...
| P1-26 | **Gmail 스코프 유지 시 CASA 보안평가 비용** | **신규(2026-08-06, 추가 82)**. `gmail.readonly`로 가져온 제목·미리보기가 소통기록에 담겨 AI 프록시로 서버에 나간다. Google은 restricted scope 데이터가 제3자 서버로 이동하면 **상위 티어 보안평가(사실상 모의해킹, 연 1회 갱신)** 대상으로 본다. **"2-0" 사용자 결정 2번(Gmail을 v1에 넣을지)이 사실상 "매년 수백만 원짜리 평가를 받을지"와 같은 질문**이라는 점이 이번에 드러났다. 정확한 티어 판정은 Google/평가기관이 하므로 **확인 필요** | 판정에 따라 대 | 사용자(결정) |
| P1-27 | **커밋 시 `analyze`+`test` 자동 실행(CI)** | **신규(2026-08-06, 추가 82)**. `.github/` 자체가 없어 테스트 39건이 **사람이 기억할 때만** 돈다. QA 개선 항목 중 비용 대비 실익 1위. GitHub Actions 또는 `pre-push` 훅 | 소 | 개발 + 사용자(방식 결정) |
| P1-28 | **배선 테스트 추가** | **신규(2026-08-06, 추가 82)**. 추가 79 결함이 정확히 이 구멍의 모양이었다 — 서비스가 재시도를 *허용*하는지는 테스트가 덮었는데 리포지토리가 *호출*하는지는 아무도 안 봤다. 현재 39건 중 위젯 테스트 1건, 통합 테스트 0건. "A가 B를 부른다" 수준의 테스트가 통합 테스트 전면 도입보다 싸고 정확하다 | 소~중 | 개발 |
| P1-29 | **실물 점검 스크립트 레포 자산화** | **신규(2026-08-06, 추가 82)**. 서버 평문 3건을 잡아낸 `check_firestore_geo.py`와 기기 저장소를 연 `check_android_local.py`가 **세션 스크래치패드에만 있어 사라졌다.** `tool/`로 옮겨 커밋. 개인정보 값은 출력하지 않고 키 존재 여부·건수만 보는 기존 원칙을 코드에 박아둘 것 | 중 | 개발 → QA(정례 실행) |
| P1-30 | **릴리스 체크리스트 문서화 + 앱 내 빌드번호 표시** | **신규(2026-08-06, 추가 82)**. 실물 점검·재설치 시나리오·빌드 신선도 확인·스토어 양식 대조 절차가 backlog 산문에만 흩어져 있다. 추가 77의 "낡은 빌드를 버그로 오인" 유형은 앱에 빌드번호(+커밋 해시)를 표시하면 구조적으로 사라진다 — P1-18과 묶으면 추가 비용 ≈ 0 | 소 | 기획 + 개발 |
