...
| P1-36 | Functions SDK 업그레이드(`firebase-functions` 7.x / `firebase-admin` 14.x) | **신규(2026-08-08)**. 배포 시 "outdated version of firebase-functions, breaking changes when you upgrade" 경고가 뜬다. `npm audit`의 moderate 8건(전부 `uuid` 하나에서 파생, `firebase-admin` → `@google-cloud/storage` → `teeny-request` 경로)도 이 업그레이드로 해소된다. 우리가 직접 쓰는 코드가 아니고 취약 조건에 해당하는 호출도 없어 급하지 않지만, 런타임과 달리 **강제 기한이 없어 미루면 계속 밀린다**. `onCall`/`HttpsError`/`logger`/`defineSecret` 사용부가 영향 범위이며 실기기 호출로 검증 필요 | 소~중 | 개발 |
