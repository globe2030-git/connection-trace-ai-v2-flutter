...
| P1-23 | 로그인하지 않은 계정의 서버 문서 정리 | 좌표·평문 제거 마이그레이션은 **그 계정으로 로그인해야** 돈다. 현재 테스트 계정 `MmNZjpID…`에 평문 1건 + 좌표 2건이 남아 있다(추가 77). 또 `rebackupAllContacts`는 **로컬에 있는 명함만** 덮어쓰므로, 로컬에 일부만 있는 기기에서 로그인하면 나머지 서버 문서는 손대지 않는다. 실사용자가 생기기 전에 남은 테스트 계정을 정리하거나 서버 측 일괄 마이그레이션(Cloud Functions, P1-7 선행) 검토 | 소~중 | 개발 + 사용자 |
