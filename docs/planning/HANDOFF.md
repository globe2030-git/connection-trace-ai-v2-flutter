...
| P0-5 | 법적 고지 문서 게시 → 🟡 **게시 완료, 스토어 등록만 남음** | **2026-08-08 실물 확인**: 4종 전부 Firebase Hosting에 배포돼 서비스 중이다(각 URL HTTP 200 확인) — `https://connection-sense.web.app/privacy-policy` / `/terms-of-service` / `/app-permissions` / `/account-deletion`. 이 문서에 오래 "배포 필요"로 적혀 있었으나 사실과 달랐다. **남은 것**: ① 스토어 콘솔(Play "앱 콘텐츠", App Store Connect)에 URL 등록 ② `connectionsense@creamhouse.net` **이메일 인증 미완료** — Firebase Auth에 `emailVerified=false`라 이 계정으로는 관리자 콘솔 로그인이 안 된다(Workspace 그룹메일 수신 문제, 2026-08-06부터 미해결) | 소 | 사용자(콘솔·메일) |
