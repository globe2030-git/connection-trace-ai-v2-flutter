...
| P1-4 | 서버측 영수증 검증 + **크레딧 지급** Cloud Function | 구독 상태 동기화 대신 **구매 영수증 검증 → `users/{uid}` 크레딧(잔여 횟수) 지급** 구조. 같은 영수증 중복 지급 방지(트랜잭션) 필수. 클라이언트가 스스로 크레딧을 못 올리게 firestore.rules도 함께 | 대 | 개발 |
