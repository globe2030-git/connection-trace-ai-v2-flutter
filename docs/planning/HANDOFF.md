...
6. **검증** — `flutter analyze`(에러 0, 기존 info 19건 외 신규 없음),
   `flutter test`(39/39 통과)까지는 완료. 실기기 시각 확인은 부분적으로만
   가능했다(아래 "실기기 검증 한계" 참고).

**실기기 검증 한계 (아침에 확인 필요)**:
- **Android**: 실기기(SM F966N, Galaxy Z Fold)에 debug APK를 설치하고
  실행해 logcat에 크래시가 없는 것까지 확인했다. 하지만 기기가 접힌
  채였고(커버 화면에 삼성 "오동작 방지 필터"가 떠 있음, 이건 조도/근접
  센서로 판단하는 기능이라 adb 터치로는 못 뚫는다), 펼쳐야 화면을 볼 수
  있어서 실제 아이콘·색상 렌더링은 육안 확인을 못 했다. **아침에 폰을
  펼쳐서 한 번만 봐주면 됨.**
- **iOS**: `flutter build ios --release`(실기기용)와 `--debug --simulator`
  둘 다 빌드는 성공했다. 그런데 이 Mac엔 iOS 26.5 시뮬레이터 런타임만
  깔려 있고, `google_mlkit_text_recognition`/`google_mlkit_commons`가
  끌어오는 pod(GoogleMLKit/MLKitCommon 등)가 Apple Silicon iOS 26+
  시뮬레이터용 arm64 슬라이스를 아직 지원하지 않아(빌드 경고에 명시된
  기존 제약 — 이번 아이콘 작업과 무관, 전부터 있던 한계) 시뮬레이터
  설치가 막혔다. iPhone 실기기가 케이블로 연결돼 있지 않아 devicectl
  경로도 못 썼다. **아이폰을 케이블로 연결해 `flutter build ios --release`
  결과물을 devicectl로 설치·실행해 봐야 최종 확인이 끝난다.**
- 즉, **코드/빌드/테스트 레벨 검증은 끝났지만 "화면이 실제로 예쁘게
  나오는지"의 최종 육안 확인은 두 플랫폼 다 사용자 쪽 물리적 조건(폰
  펼치기/케이블 연결)이 필요해서 오늘 밤 안에는 못 끝냈다.**

**작업 브랜치**: `feature/icon-color-rebrand` (worktree
`connection-trace-ai-v2-flutter-work/`, `main`에서 분기). [[connection-trace-ai
auto-commit]] 규칙대로 완료 단위마다 커밋했고, `main` 병합/원격 push는
사용자 확인 후 진행 예정 — 아직 안 했다.
...
| P1-28 | 아이콘·블루 리브랜딩 실기기 육안 확인 + `main` 병합 | **신규(2026-08-05 밤, 0-4 섹션)**. 코드/빌드/테스트는 끝났지만 Android는 폰이 접힌 채라(오동작 방지 필터) 화면을 못 봤고, iOS는 이 Mac의 시뮬레이터(iOS 26.5)가 google_mlkit pod의 arm64 미지원으로 못 띄우고 실기기도 케이블이 안 꽂혀 있어 실행을 못 함. 아침에 ① 안드로이드 폰 펼쳐서 육안 확인 ② 아이폰 케이블 연결 후 `flutter build ios --release` 결과물 devicectl 설치·실행, 둘 다 확인되면 `feature/icon-color-rebrand` → `main` 병합 + push | 소(확인만) | 사용자(폰 펼치기/케이블) + 개발(병합) |
