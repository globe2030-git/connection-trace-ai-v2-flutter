...
| P2-10 | `npm audit` moderate 7건(`uuid`) — 상류 대기 | **신규(2026-08-08)**. 전부 `uuid@9.0.1` 하나에서 파생된다: `firebase-admin@14.2.0`(최신) → `@google-cloud/storage@7.21.0` → `gaxios`/`teeny-request` → `uuid`. 수정본은 `uuid` ≥ 11.1.1인데 **상류가 아직 안 올려서 최신 버전을 써도 해소되지 않는다.** `npm audit fix`가 제시하는 해결책은 `firebase-admin`을 10.3.0으로 **다운그레이드**하는 것이라 따르면 안 된다. 실질 위험은 낮다 — 취약 조건은 `buf` 인자를 넘기는 `uuid` v3/v5/v6 호출인데 우리는 `uuid`를 직접 쓰지 않고, 경로상 상위인 Cloud Storage도 이 함수에서 쓰지 않는다. `overrides`로 강제 상향할 수 있지만 우리가 쓰지도 않는 코드 경로를 위해 상류와 어긋난 트리를 유지하는 비용이 더 크다고 판단했다. **상류가 올릴 때까지 대기** | 소(대기) | 개발 |
