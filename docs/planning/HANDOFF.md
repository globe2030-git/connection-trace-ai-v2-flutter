...
6. **베타 관측 체계 — 4건, 전부 권장안 있음(2026-08-12 설계 완료, 추가
   170, `docs/planning/beta-observability-plan.md` 8절).** ① GA(Firebase
   Analytics) 도입 시점 — 지금 베타 중 vs 정식 출시 준비 단계(권장: 후자,
   근거는 문서 5절). ② 인구통계(성별/연령/지역) 수집 방식 — 가입 시 직접
   입력 vs GA 추론 vs 지금은 안 함(권장: 지금은 안 함). ③ `latestBuild`
   갱신 시점 — 지금 바로 vs 다른 세션 테스트 종료 확인 후(권장: 후자 —
   다른 세션이 아직 빌드 6으로 활성 테스트 중이라 지금 올리면 방해가 됨).
   ④ 베타 종료 최소 기간 — 권장안은 날짜를 못 박지 않고 표본 요건(성공
   호출 150건↑·참여 uid 5명↑·영업일 7일↑) 충족 여부로 판단. 이견 없으면
   권장안대로 진행 가능.
