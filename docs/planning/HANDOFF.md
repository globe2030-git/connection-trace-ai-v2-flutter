# 개발 인수인계 문서 (2026-08-09 기준 — 최신 상황은 "0-9" 섹션, 우선순위는 "3. 해야 할 일" 참고)
...
- **iOS 외부 테스트 그룹** — 테스터 이메일 수집 후 생성(내일). iOS는 TestFlight
  빌드라 App Attest로 AI가 통과되므로 허용목록 없이도 됨.
...
| P1-38 | **Apple 토큰 폐기(revoke)** → 🟡 **코드 완료, 배포 대기(2026-08-09, 추가 114, PR #44)** | 로그인 시 authorizationCode를 서버가 refresh_token으로 교환해 `appleAuth/{uid}`(클라이언트 접근 불가)에 보관, 탈퇴 시 `onUserDeletedCleanup` 트리거가 Apple `/auth/revoke` 호출. ES256 JWT는 Node crypto로 직접 서명, client_id=번들ID. **⚠️ 배포 전 사용자 작업**: `firebase functions:secrets:set APPLE_SIGNIN_KEY`(.p8 내용) → `firebase deploy --only functions,firestore:rules` → 실기기 검증(Apple로그인→계정삭제→Apple ID 설정에서 앱 제거 확인) | 배포·검증 | 사용자(secret) + 개발 |
