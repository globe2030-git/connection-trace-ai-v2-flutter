...
| P1-38 | **Apple 토큰 폐기(revoke) 미구현** | **신규(2026-08-08)**. Apple은 Sign in with Apple + 계정 삭제 기능이 있는 앱에 **토큰 폐기**를 요구한다. 우리 앱은 둘 다 해당되는데 `revokeToken` 호출이 코드에 없다(`grep` 확인). 필요한 것: ① Services ID 생성 ② Firebase OAuth 코드 흐름 설정(키 `UUYAKPD4S7` 재사용) ③ 계정 삭제 시 폐기 호출. **정식 심사 전 처리** | 소~중 | 개발 + 사용자(콘솔) |
