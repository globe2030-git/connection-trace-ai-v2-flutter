# 개발 인수인계 문서 (2026-08-11 기준 — 최신 상황은 "0-11" 섹션, 우선순위는 "3. 해야 할 일" 참고)
...
## 0-11. 2026-08-11 — 심사 통과·과금 확정·충전 관리 콘솔·에셋 구조화 (가장 최신)
...
| P1-2 | 인앱결제(IAP) 클라이언트 연동(`in_app_purchase`) — **소모성 구매 + 소진(consume) 처리** | 소모성 상품은 구매 후 소진 처리를 해야 재구매가 가능하다. 충전 UI(잔여 횟수 표시 + 충전 버튼)는 기존 "AI 잔여 횟수" 화면에 얹는 것이 자연스러움 | 중 | 개발 |
