# 개발 인수인계 문서 (2026-08-12 기준 — 최신 상황은 "0-15" 섹션, 우선순위는 "3. 해야 할 일" 참고)
...
## 0-15. 2026-08-12 — 베타 관측 체계 기획 (문서만, 가장 최신)
...
**🆕 2026-08-12 — 빌드 7 테스터 피드백 25건 처리 계획 추가됨(코드 변경 없음,
사용자 검토 대기)**: 아래 P0/P1/P2 표는 2026-08-04 기준이라 이번 테스터
피드백(E-01~E-10·F-01~F-10·O-01~O-02, 22건)과 코디네이터가 조사 중 추가
지시한 3건(**E-11** — 명함 등록 시 태그가 `AI, IT`로 자동 채워지는 가짜
데이터 버그, **F-11** — 부서·회사명 기반 태그 추천 미확정 설계, **F-12** —
부서 필드 신설·사용자 확정)이 반영돼 있지 않다. 상세 계획은
`docs/planning/beta-feedback-build7-plan.md`(backlog 추가 175 요약 참고).
E-05·E-10은 이미 수정·병합 완료. **E-11은 "가짜 데이터 금지" 원칙 위반으로
1차 최상단 우선순위.** **F-12는 이 계획 전체에서 유일하게 저장 스키마를
바꿔 전체 테스트(실기기)가 필요한 항목**이라 의도적으로 2차로 분리했다.
나머지는 실행 순서(1차/2차/보류)까지 정리돼 있으니, 다음에 이 항목들을
다룰 때는 그 문서를 먼저 볼 것 — 착수 전 사용자 결정 필요 항목(F-02
필수항목 완화 범위, F-05 패키지 도입 여부, F-11 추천 표시 방식, E-11 기존
오염 태그 처리 방침 등)이 있다.
