...
- 회원탈퇴 흐름(즉시 삭제).
- **AI 프록시 호출 한도 초과 시나리오(일 10회/월 100회 등, 실제 채택값
  기준) — 한도 도달 시 안내 문구가 뜨고 요청이 차단되는지 확인
  (2026-08-04 추가).**
...
- [ ] 발급한 키를 Cloud Functions 환경변수/Secret Manager에 등록(개발자가
      ...
      방법으로 전달)
- [ ] 사용자당 AI 호출 한도 제안값(일 10회/월 100회, `server-setup-plan.md`
      14.4절)을 확인하고, 다르게 원하면 조정 요청
