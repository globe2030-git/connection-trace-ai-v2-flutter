 ...
 * §1에서 사용자가 확정한 "가입 체험 회차" 값이다(안정기엔 5회로 환원
 * 가능하도록 config로 뺀 것 — 그 config가 비어 있을 때의 폴백이 바로 이 상수).
 *
 * 📌 **2026-08-18에 10 → 20으로 올렸다**(사용자 확정). 같은 날 *"지금 모델의
 * 하루 무료 호출 한도는 없는 것으로 한다"*는 결정과 한 쌍이다 — 하루 한도
 * (index.ts의 `DAILY_LIMIT`)를 없애는 대신, **1회성 무료분이 유일한 무료
 * 상한**이 된다. 그래서 회차를 두 배로 늘렸다.
 *
 * ⚠️ **순서가 중요하다.** 하루 한도 제거는 이 지갑 모델이 배포된 뒤여야
 * 한다 — 먼저 지우면 상한이 아예 없어져 비용이 샌다.
 *
 * 💰 비용 영향(계산, 실측 아님): pnl 문서의 실측 단가 $0.0023/회 기준으로
 * 가입자 1인당 무료분 원가가 $0.023 → **$0.046**(약 65원)이 된다.
 * 손익분기 전환율도 그만큼 올라가므로 pnl 재계산이 필요하다.
 */
