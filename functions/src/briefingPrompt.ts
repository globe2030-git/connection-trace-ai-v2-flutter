...
포인트 문장만 줄바꿈으로 구분해서 정확히 3개 작성하세요.`;
  ...
  //
  // ⚠️ "관계·근황" 축은 **재료가 없을 수 있다.** 새로 등록한 명함은 소통
  // 기록이 0건이다. 그때 모델이 할 수 있는 것은 셋인데 — 축을 비우거나(문장이
  // 둘이 되어 "정확히 3개"와 부딪힌다), 다른 축으로 채우거나, **지어내거나**
  // — 셋 중 지어내는 것이 가장 그럴듯해 보인다. "지난번에 말씀하신
  // 프로젝트는…"은 소통 기록이 없어도 자연스럽고, 이용자는 그게 지어낸
  // 것인지 알 수 없다. 그래서 비었을 때 무엇을 하라고 명시한다.
  //
...
- 관계·근황: 위 [최근 소통 기록]과 [상대방 정보]에 실제로 있는 내용에 기댄 근황.
  기댈 내용이 없으면(예: "최근 소통 기록 없음") 이 축을 억지로 채우지 말고, 그
  자리를 가벼운 안부나 분야 화제로 대신하세요. 없었던 일을 있었던 것처럼 쓰면
  안 됩니다. 축을 대신하더라도 문장은 그대로 3개입니다.
