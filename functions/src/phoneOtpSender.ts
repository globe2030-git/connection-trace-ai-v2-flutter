 ...
 *
 * 🚨 **2026-09-06부터 실제로는 안 나간다** — `buildAligoSendPayload` 가
 * `failover: "N"` 을 준다(그 자리 주석에 경위가 있다). **상수를 지우지 않은
 * 것은 의도다** — 되돌릴 때 다시 만들지 않으려고 남겨 뒀다.
 *
 * ⚠️ **승인된 알림톡 템플릿에는 이 대체문자가 등록된 채로 남아 있다.**
 * 템플릿에는 있고 우리가 안 쓰는 상태다.
 */
...
export function buildAligoSendPayload(
  ...
  return {
    ...
    // 🚨 **대체문자를 끈다 — 인증은 카카오톡으로만 보낸다**
    // (2026-09-06, globe2030님 결정).
    //
    // ## 왜
    //
    // ```
    // 카카오톡 국내 MAU   5,534만 (2026 2분기) · 인터넷 이용자의 97.1%
    // 얻는 것             발신번호 사전등록(서류 + 심사 1~3일)이 통째로 사라진다
    // ```
    //
    // ⚠️ **3% 미만이지만 0명이 아니다.** 그리고 인증에 건너뛰기가 없으므로
    // (추가 560) **그 사람들은 둘러 갈 길이 없다.** globe2030님이 그 대가를
    // 알고 고르셨다.
    //
    // ## 🚨 껐지 지우지 않았다 — 되돌리는 법
    //
    // 아래 `fsubject_1`·`fmessage_1` 과 `OTP_FAILOVER_SUBJECT`·
    // `OTP_FAILOVER_BODY`, 그리고 `ALIGO_SENDER` 시크릿을 **전부 남겨 뒀다.**
    // 되돌리려면 **이 한 줄을 `"Y"` 로 바꾸고 배포 한 번**이면 된다.
    //
    // 📌 지금은 테스터 단계라 되돌리기가 싸다. 출시 뒤 *"카톡 없는 사람이
    // 가입을 못 한다"* 는 제보가 오면 그 자리에서 켠다. **지우면 그때 다시
    // 만들어야 한다.**
    //
    // ⚠️ **알리고가 이 필드를 요구하므로 값 자체는 계속 보낸다** — `"N"` 이면
    // 쓰이지 않을 뿐이다. `ALIGO_SENDER` 도 `pickSender()` 가 다섯 중 하나로
    // 요구하고 알리고 payload 의 필수 필드라 **여전히 필요하다.**
    //
    // 🚨 **승인된 템플릿에는 대체문자가 등록된 채로 남아 있다**
    // (globe2030님 화면 확인 2026-09-06: *"대체 발송 문자: 단문 —
    // [커넥션센스] 인증번호 #{인증번호} …"*). 즉 **템플릿에는 있고 우리가
    // 안 쓰는 상태**다. 템플릿을 고치려면 재심사(영업일 2일)라 건드리지
    // 않는다 — **다음 사람이 「템플릿에 있으니 나가겠지」로 읽지 않도록 여기
    // 적어 둔다.**
    //
    // ⚠️ 옛 주석을 남긴다: *"템플릿 신청에서 대체문자를 켰더라도 여기서 Y를
    // 줘야 한다(공식 예제 주석)"* — **그 사실은 그대로다.** 바뀐 것은 우리가
    // 대체문자를 **안 쓰기로 한 것**이지 알리고의 규칙이 아니다.
    failover: "N",
