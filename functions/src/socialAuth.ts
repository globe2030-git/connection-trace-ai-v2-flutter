 ...
 * ```
 *
 * ⚠️ `id`가 **숫자로 온다.** 받자마자 문자열로 고정한다 — uid는 문자열이어야
 * 하고, 숫자로 들고 다니면 다루는 곳마다 형이 갈린다.
 *
 * ⚠️ **다만 "정밀도를 지킨다"고 말할 수는 없다.** 한때 그렇게 적어 뒀는데
 * 사실이 아니다. `res.json()`이 이미 자바스크립트 double 로 바꾼 **뒤**에
 * 우리가 받으므로, 2^53(9007199254740992)을 넘는 회원번호였다면 **여기
 * 도달하기 전에 이미 반올림돼 있다.** 문자열로 바꾸는 것은 그 뒤의 일이라
 * 되돌리지 못한다. (그 주장을 지킨다던 테스트도 리터럴 자체가 반올림돼
 * 있어서 아무것도 증명하지 못했다.)
 *
 * 📌 **지금은 문제가 아니다** — 카카오 회원번호는 10자리 안팎이라 2^53과는
 * 자릿수가 여섯 자리쯤 차이 난다. 진짜로 지켜야 할 날이 오면 응답을 텍스트로
 * 읽어 `"id":<숫자>`를 정규식으로 꺼내야 한다. **없는 보장을 있다고 적어 두면
 * 다음 사람이 확인 없이 믿는다.**
 *
 ...
 *
 * ⚠️ **빈 값이면 여기서 던진다.** 한때 *"시크릿을 꺼 둔 옛 앱도 있으니 값이
 * 있을 때만 싣는다"* 고 적어 뒀는데, 그 아래 코드는 빈 값일 때 이미 예외를
 * 던지고 있어서 **주석과 코드가 서로 달랐다.** 운영자가 주석을 믿으면
 * "시크릿 없이도 도는 줄" 알고 설정 누락을 엉뚱한 데서 찾게 된다.
 * 설정이 빠진 것을 이름을 붙여 드러내는 편이 낫다.
 */
export function tokenExchangeBody(params: {
  ...
  const body = new URLSearchParams({
    ...
    // 위에서 빈 값을 이미 걸렀으므로 여기서 다시 검사하지 않는다.
    // (예전에는 `if (clientSecret?.trim())`로 감싸 뒀는데, 위 검사 때문에
    //  **항상 참인 죽은 조건**이었다. 읽는 사람에게 "없을 수도 있다"는
    //  잘못된 인상을 준다.)
