/// 전화 걸 번호를 고르는 바텀시트.
///
/// 2026-08-26에 `core/services/phone_call_service.dart`에서 **한 줄도 고치지
/// 않고** 옮겨 왔다(추가 505).
///
/// 왜 옮겼나: 그 파일은 `core`에 있으면서 `presentation/common/glass_card.dart`를
/// import하고 있었다 — `lib/core/` 아래에서 `presentation`을 아는 **유일한
/// 파일**이었다(실측 1건). 그런데 import를 옮기는 것으로는 안 되는 문제였다.
/// 231줄 중 **128줄이 이 위젯 트리**였다. import는 증상이고 원인은 화면이
/// core에 있는 것이었다.
///
/// 호출부에 올리지 않고 여기 둔 이유: 부르는 곳이 **넷**이라(briefing_overlay ·
/// wallet_view · nearby_map_view · radar_view) 각자에게 올리면 128줄이 네 벌이
/// 된다. 계층은 지키고 중복을 만드는 셈이라 더 나쁘다. presentation 안의 공용
/// 위치 한 곳이면 계층 방향(presentation → core)도 지켜지고 중복도 안 생긴다.
///
/// 실제 걸기(`tel:` 열기)는 그대로 [PhoneCallService.makeCall]이 한다.
library;
...
Future<bool> showCallPicker(
  ...
  return PhoneCallService.makeCall(selected);
}
