...
class _AddCardModalViewState extends State<AddCardModalView> {
  ...
  Future<void> _saveCard() async {
    ...
    // 1-2. 이미 저장된 명함 중 **주소가 글자 그대로 같은** 것이 있으면 그
    // 좌표를 그대로 쓴다(globe2030님 지적, 2026-08-28).
    //
    // > "회사별 좌표는 한번만 확인되면 저장되서 명함의 회사 주소로 계속
    // >  검색할 이유가 없지않나?"
    //
    // 🚨 **이 지적이 가리킨 곳이 바로 여기다.** 좌표 채우기(백필)에는 같은
    // 손을 이미 댔는데, 등록은 저장하며 좌표가 바로 붙어 그 경로에 아예
    // 가지 않는다 — 같은 회사 명함 다섯 장이면 **같은 주소를 다섯 번**
    // 물어보고 있었다.
    //
    // ## ⚠️ 좌표만이 아니라 도로명 변환도 함께 건너뛴다
    //
    // 빌려오는 조건이 **"저장된 주소와 정확히 일치"**다. 그러면 이렇게 된다.
    //
    // ```
    // A가 변환을 거쳐 저장됨   → 일치하는 B도 이미 변환된 형태다 (변환할 것이 없다)
    // A가 변환 없이 저장됨     → B도 같은 형태로 남는다 (A와 B가 일관된다)
    // ```
    //
    // 📌 **B만 변환해서 문자열이 달라지면 다음 명함이 둘 중 어느 것과도 안
    // 맞는다** — 좌표만 빌리고 변환은 하는 쪽이 오히려 이 재사용을 깨뜨린다.
    //
    // ⚠️ **잃는 것도 있다**: A가 변환 안 된 채로 저장돼 있고 B는 변환하면
    // 좋아졌을 경우, 그 개선을 놓친다. 그래도 **A와 B가 갈라지는 것보다
    // 낫다**고 봤다(PM 검토).
    //
    // 📌 주소 검색으로 고른 좌표(`_pickedGeo`, 바로 위)와 **같은 취급**이다 —
    // 그쪽은 "이미 검증된 주소"라 변환을 다시 묻지 않고(추가 83), 이쪽은
    // "이미 이 앱을 통과해 저장된 주소"다.
    final reusedGeo = context.read<WalletViewModel>().geoForSameAddress(
      rawAddress,
    );
    if (reusedGeo != null) {
      // 집계는 저장소가 센다(`geoForSameAddress`) — 화면이 세면 다른 호출부가
      // 생겼을 때 빠뜨린다. 판정과 집계를 한 곳에 둔다.
      _executeFinalSave(rawAddress, reusedGeo);
      return;
    }
