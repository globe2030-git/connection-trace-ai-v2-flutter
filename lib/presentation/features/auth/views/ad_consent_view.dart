...
class AdConsentView extends StatefulWidget {
  ...
  /// 답을 제출했을 때 **화면을 닫을지. 기본값은 `true`(닫는다).**
  ///
  /// 🚨 **기본값을 2026-09-03 에 `false` → `true` 로 뒤집었다 — 같은 결함이
  /// 세 번째로 났기 때문이다.** 아래 ①②는 「첫 물음」 경로에서 겪은 것이고,
  /// **③ 은 「설정에서 여는 경로」에서 같은 증상이 그대로 반복된 것**이다:
  /// `settings_view` 가 이 값을 **안 넘겨서** 기본값 `false` 가 먹었고,
  /// **저장은 되는데 화면이 안 닫혔다.**
  ///
  /// ⚠️ **설정 경로가 오히려 더 나쁘다.** 첫 물음은 못 나가면 앱에 들어갈 수가
  /// 없어 **바로 티가 나는데**, 설정은 뒤로가기로 나갈 수 있어 이용자에게는
  /// *"저장이 안 됐나?"* 로만 보인다 — **실제로는 저장됐는데 다시 누른다.**
  /// 게다가 `settings_view._open()` 의 `setState` 는 **화면이 닫혀야** 도는
  /// 자리라(`await Navigator.push` 뒤), 설정 화면의 요약 줄도 안 바뀐다.
  ///
  /// 📌 **그래서 「닫는다」를 기본으로 두고, 안 닫는 쪽을 예외로 적게 했다.**
  /// 종전에는 반대라 **부르는 자리마다 기억해야 했고, 빠뜨리면 조용히**
  /// 안 닫혔다. 지금은 빠뜨려도 닫힌다.
  ///
  /// ⚠️ `false` 로 두려면 **왜 안 닫는지를 그 자리에 적을 것.**
  ///
  ...
  ///
  /// ## 이제는 둘 다 닫는다 (2026-09-03)
  ///
  /// | | 제출했을 때 |
  /// |---|---|
  /// | **첫 물음** | 성공이든 실패든 **닫는다** — 앱을 못 쓰게 막으면 안 된다 |
  /// | **설정** | 성공이든 실패든 **닫는다** — 저장했는데 안 닫히면 안 한 줄 안다 |
  ///
  /// ⚠️ 종전에는 설정에서 **머물게** 두었다. *"바꾸러 들어온 사람이니 결과를
  /// 화면에서 본다"* 는 뜻이었는데, **실제로는 결과가 화면에 안 나타났다** —
  /// 성공하면 체크 상태가 그대로라 **바뀐 것이 아무것도 안 보인다.**
  ///
  /// 실패해도 닫는 것이 안전하다. 저장이 안 됐으니 **동의하지 않은 상태**로
  /// 남고, 그 상태에서는 아무것도 보내지 않는다. 설정 화면의 요약 줄도
  /// 안 바뀌므로 **화면과 서버가 어긋나지 않는다.**
...
class _AdConsentViewState extends State<AdConsentView> {
  ...
  Future<void> _submit() async {
    ...
    // 🚨 **실패해도 닫는다.** 첫 물음에서 머물면 앱에 들어갈 길이 없고,
    //    설정에서 머물면 저장이 안 된 것처럼 보여 이용자가 다시 누른다.
    ...
    // 🚨 **`dismissOnSubmit: false` 를 일부러 넘긴 자리만 여기로 온다.**
    // 2026-09-03 에 기본값이 `true` 로 바뀌어 지금은 부르는 곳이 없다 —
    // 지우지 않고 남겨 둔다. 「저장 실패인데 화면은 머물러야 하는」 화면이
    // 나중에 생길 수 있고, 그때 되돌리는 규칙이 여기 적혀 있다.
    //
    // 화면을 **들어올 때의 값으로 되돌리고** 이유를 알린다. 무조건 false 로
    // 내리면 설정에서 켜져 있던 사람에게 "꺼진 것처럼" 보여 서버 상태와
    // 화면이 반대로 어긋난다.
    setState(() {
