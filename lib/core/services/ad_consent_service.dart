...
///
/// ## 🚨 다른 동의 서비스를 그대로 베끼면 안 된다
///
/// 이 앱에는 **사진 개선 동의**(`PhotoImprovementConsentService`)가 있었다.
/// 겉모습이 이것과 거의 같았다 — 기기 캐시 + `users/{uid}` 필드, 서버 실패 시
/// 되돌리기. 그래서 **복사해서 고치기 쉬운 자리**였는데, **두 곳이 반대여야
/// 했다.**
///
/// | | 사진 개선 동의(걷힘) | **광고 수신 동의(이 서비스)** |
/// |---|---|---|
/// | 철회하면 동의 시각을 | `null`로 **지웠다** | 🚨 **지우지 않는다** |
/// | 읽기 실패는 | 언제나 "동의 안 함" | **쓰는 곳에 따라 다르다**(아래) |
///
/// ⚠️ **그 서비스는 2026-09-05에 걷혔다**(추가 688 — 동의만 받고 읽는 곳이
/// 0건이었다). **그래도 이 문단은 남긴다** — 다음에 새 동의를 만들 때
/// **여기를 베끼려는 사람**에게 필요한 것은 *"어느 파일을 베끼지 마라"* 가
/// 아니라 **왜 광고 동의만 다른가**이고, 그 이유는 아래 ①②에 그대로 있다.
///
