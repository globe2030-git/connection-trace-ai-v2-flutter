...
class ContactImageService {
  ...
  /// **장수만큼 서버를 읽는다**(실측 102건이면 102회). 그래서 도는
  /// 쪽이 한 번 읽어 넘긴다.
  ///
  ...
  /// 올린다.
  ///
  /// 감수한 크기: **실측 16.44MB**(102건 · 평균 165KB, 2026-08-26 Cloud Storage
  /// 조회). 한도(2,000장)까지 차 있으면 이론상 그보다 훨씬 크다.
  ///
  /// ⚠️ **처음에는 24MB로 적었는데 틀렸다. 경위를 남긴다** — 결론만 고치면
  /// 다음 사람이 같은 자리에서 같은 추정을 한다.
  ///
  /// ```
  /// 처음 적은 값   103장 × 236KB ≈ 24MB   ("둘 다 실측"이라고 썼다)
  /// 실물           102건 × 165KB = 16.44MB
  /// ```
  ///
  /// 틀린 곳이 둘이다. ① **236KB는 재인코딩 표본의 평균**이었고 실제 저장분은
  /// 그보다 작았다 — 같은 이름의 "평균"이라도 무엇을 잰 평균인지가 다르다.
  /// ② **103은 명함 수였고 사진은 102건**이다. 사진 없이 등록한 명함이 하나
  /// 있다. 명함 수를 사진 수로 쓴 것은 비약이었다.
  ///
  /// 📌 **"둘 다 실측"이라고 적었지만 실은 다른 것을 잰 값을 곱한 계산이었다.**
  /// 계산은 틀려도 그럴듯해 보이고 실측은 틀리면 티가 난다(CLAUDE.md 4절).
  ///
