...
class EncryptionKeyService {
  ...
  /// **이 계정의 명함을 열 수 있는 키들** — 앞의 것부터 시도한다 (키링, 2026-09-04).
  ///
  /// ## 지금은 하나다. 그런데 자리를 미리 낸다
  ///
  /// 한 사람이 계정을 둘 이상 갖게 되면(로그인 수단을 더하거나 번호로 잇거나)
  /// 명함이 **서로 다른 키로 잠긴 채** 한 명함첩에 모인다. 그때 필요한 것이
  /// 「가진 키를 차례로 시도하는」 능력이고, 그 시도는
  /// [DataCryptoService.decryptJsonWithAny]가 한다.
  ///
  /// 📌 **오늘 이 함수는 키 하나짜리 목록을 돌려준다 — 동작이 하나도 안
  /// 바뀐다.** 바뀌는 것은 **부르는 쪽의 모양**이다. 나중에 키링이 생기면
  /// **여기 한 군데만** 고치면 된다.
  ///
  /// ⚠️ **왜 미리 내는가**: uid 를 어떻게 통일할지(A·B·C 안)가 아직 안
  /// 정해졌는데, 그 사이에도 가입은 계속되고 명함은 계속 쌓인다. 결정을
  /// 기다리는 동안 **키 하나를 전제로 쓴 코드가 늘어나면** 나중에 고칠 곳이
  /// 그만큼 많아진다.
  ///
