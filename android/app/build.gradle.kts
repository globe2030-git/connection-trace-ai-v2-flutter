...
android {
    ...
    packaging {
        jniLibs {
            ...
            //
            // 🚨 **2026-08-26에 `armeabi-v7a`(32비트)도 뺐다**(사용자 결정).
            // 위 x86_64와 이유가 다르다 — 저쪽은 "에뮬레이터용이라 실기기에
            // 안 쓴다"이고, 이쪽은 **"실기기인데 우리가 검증하지 못한다"**다.
            // 테스트 기기가 폴드(64비트)뿐이라 32비트에서만 나는 결함을 볼
            // 사람이 없다. 되돌리는 비용도 한쪽으로 기울어 있다: 나중에 넣는
            // 것은 무해하지만, 넣고 시작했다가 빼면 이미 깔린 이용자의
            // 업데이트가 끊긴다.
