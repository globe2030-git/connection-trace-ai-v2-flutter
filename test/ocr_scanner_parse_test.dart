...
void main() {
  ...
  group('회사 로고 접두 (추가 318)', () {
    ...
    test('⚠️ 3자 이상 접두는 안 뗀다 — 글자로 인쇄된 이름이다', () {
  ...
  group('상세주소의 영문 로고 잔재 (테스터 제보, 2026-08-20)', () {
    test('2단 레이아웃이 한 줄로 뭉쳐 브랜드명이 층수 뒤에 붙어도 뗀다', () {
      // ML Kit이 서로 다른 열의 텍스트를 같은 행으로 합치면("5층" 옆에
      // 로고체 브랜드명이 함께 읽힘) 예전엔 줄 전체를 그대로 상세주소로 썼다.
      final r = parse([
        '서울특별시 강남구 테헤란로 401',
        '5층 ARCTERYX',
        '02-555-1234',
      ...
      expect(r.addressDetail, isNot(contains('ARCTERYX')));
    });

    test('한글 부서명 꼬리는 그대로 둔다 — 회귀 확인', () {
      final r = parse([
        '테스트(주)',
        '홍길동',
        '서울특별시 강남구 테헤란로 123',
    ...
    test('숫자·기호가 섞인 토큰은 손대지 않는다 — 회귀 확인', () {
      final r = parse([
        '테스트(주)',
        '홍길동',
        '06193 서울특별시 강남구 테헤란로 123, 5층 501호',
