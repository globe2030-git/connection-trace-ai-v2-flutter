...
void main() {
  group('확정값 — 바뀌면 비용 계산이 어긋난다', () {
    test('무료 200 · 충전 2,000 · 경고선 80%', () {
      // 이 값들은 비용 스펙(card-photo-storage-cost-spec-2026-08-16.md)의
      // 계산 전제다. 바꾸려면 그 문서의 원가도 함께 고쳐야 한다.
      expect(kFreeCardPhotoQuota, 200);
      expect(kChargedCardPhotoQuota, 2000);
  ...
  group('resolveQuota — 서버 값이 우선, 상수는 기본값', () {
    ...
    test('없으면 기본값', () {
      expect(resolveQuota(null), kFreeCardPhotoQuota);
    });

    test('0·음수는 "백업 끄기"가 아니라 잘못된 값으로 본다', () {
      // 그런 값 때문에 백업이 조용히 멈추면 사용자는 이유를 알 수 없다.
      expect(resolveQuota(0), kFreeCardPhotoQuota);
      expect(resolveQuota(-10), kFreeCardPhotoQuota);
    });
  });
